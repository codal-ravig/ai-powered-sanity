    ...
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-900 via-slate-900 to-black text-white px-6 py-20">
      <div className="container mx-auto max-w-5xl">
        ...
        <header className="mb-16">
          <h1 className="text-6xl font-extrabold tracking-tight mb-4 bg-gradient-to-r from-cyan-400 to-cyan-600 bg-clip-text text-transparent">Our Bakery Locations</h1>
          ...
          {locations.map((location) => (
            <Link key={location._id} href={`/locations/${location.slug?.current}`} className="group relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/5 p-2 transition-all hover:bg-white/10 hover:shadow-[0_0_50px_-10px_rgba(6,182,212,0.3)] backdrop-blur-xl">
              <div className="relative aspect-video w-full overflow-hidden rounded-[2rem]">
                {location.image ? (
                  <img src={location.image} alt={location.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                ) : (
